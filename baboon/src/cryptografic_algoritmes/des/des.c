...
static const int ip_table[] = { 58, 50, 42, 34, 26, 18, 10, 2, 60, 52, 44, 36, 28, 20, 12, 4, 62, 54, 46, 38, 30, 22, 14, 6, 64, 56, 48, 40, 32, 24, 16, 8, 57, 49, 41, 33, 25, 17, 9, 1, 59, 51, 43, 35, 27, 19, 11, 3, 61, 53, 45, 37, 29, 21, 13, 5, 63, 55, 47, 39, 31, 23, 15, 7 };

/*******************************/
/* Final DES permutation table */
/* this inverts the ip_table */
/*******************************/
static const int fp_table[] = { 40, 8, 48, 16, 56, 24, 64, 32, 39, 7, 47, 15, 55, 23, 63, 31, 38, 6, 46, 14, 54, 22, 62, 30, 37, 5, 45, 13, 53, 21, 61, 29, 36, 4, 44, 12, 52, 20, 60, 28, 35, 3, 43, 11, 51, 19, 59, 27, 34, 2, 42, 10, 50, 18, 58, 26, 33, 1, 41, 9, 49, 17, 57, 25 };

/*********************************************************************/
/* Key transformation table. */
/* 64-bit key to 56-bit key, this table describe this transformation */
/*********************************************************************/
static const int permutation_table1[] = { 57, 49, 41, 33, 25, 17, 9, 1, 58, 50, 42, 34, 26, 18, 10, 2, 59, 51, 43, 35, 27, 19, 11, 3, 60, 52, 44, 36, 63, 55, 47, 39, 31, 23, 15, 7, 62, 54, 46, 38, 30, 22, 14, 6, 61, 53, 45, 37, 29, 21, 13, 5, 28, 20, 12, 4 };
...
/*************************************************************************/
static const int permutation_table2[] = { 14, 17, 11, 24, 1, 5, 3, 28, 15, 6, 21, 10, 23, 19, 12, 4, 26, 8, 16, 7, 27, 20, 13, 2, 41, 52, 31, 37, 47, 55, 30, 40, 51, 45, 33, 48, 44, 49, 39, 56, 34, 53, 46, 42, 50, 36, 29, 32, };

/********************************************************************/
/* Expantions table. */
/* shows wich output positions corresponds to with input position. */
/********************************************************************/
static const int expansion_table[] = { 32, 1, 2, 3, 4, 5, 4, 5, 6, 7, 8, 9, 8, 9, 10, 11, 12, 13, 12, 13, 14, 15, 16, 17, 16, 17, 18, 19, 20, 21, 20, 21, 22, 23, 24, 25, 24, 25, 26, 27, 28, 29, 28, 29, 30, 31, 32, 1, };

/*********************************/
/* final input block permutation */
/*********************************/
static const int p_table[] = { 16, 7, 20, 21, 29, 12, 28, 17, 1, 15, 23, 26, 5, 18, 31, 10, 2, 8, 24, 14, 32, 27, 3, 9, 19, 13, 30, 6, 22, 11, 4, 25 };
...
static void des_block_operate(const unsigned char plaintext[DES_BLOCK_SIZE], unsigned char ciphertext[DES_BLOCK_SIZE], const unsigned char key[DES_KEY_SIZE], operation_type operation) {
	...
	permute(pc1key, key, permutation_table1, PC1_KEY_SIZE);
	for (ronde = 0; ronde < 16; ronde++) {
		...
		permute(subkey, pc1key, permutation_table2, SUBKEY_SIZE);
...
static void des_operate(const unsigned char *input, int input_len, unsigned char *output, const unsigned char *iv, const unsigned char *key, operation_type operation, int trip) {
	...
	while (input_len) {
		...
		if (operation == DECRYPT) {
			...
			if (trip) {
				des_block_operate(input_block, output, key + (DES_KEY_SIZE * 2), operation);
				memcpy(input_block, output, DES_BLOCK_SIZE);
				des_block_operate(input_block, output, key + DES_KEY_SIZE, ENCRYPT);
				memcpy(input_block, output, DES_BLOCK_SIZE);
				des_block_operate(input_block, output, key, operation);
...
void des_encrypt(const unsigned char *plaintext, const int plaintext_len, unsigned char *ciphertext, void *iv, const unsigned char *key) {
	des_operate(plaintext, plaintext_len, ciphertext, (const unsigned char *) iv, key, ENCRYPT, 0);
}

void des3_encrypt(const unsigned char *plaintext, const int plaintext_len, unsigned char *ciphertext, void *iv, const unsigned char *key) {
	des_operate(plaintext, plaintext_len, ciphertext, (const unsigned char *) iv, key, ENCRYPT, 1);
}

void des_decrypt(const unsigned char *ciphertext, const int ciphertext_len, unsigned char *plaintext, void *iv, const unsigned char *key) {
	des_operate(ciphertext, ciphertext_len, plaintext, (const unsigned char *) iv, key, DECRYPT, 0);
}

void des3_decrypt(const unsigned char *ciphertext, const int ciphertext_len, unsigned char *plaintext, void *iv, const unsigned char *key) {
