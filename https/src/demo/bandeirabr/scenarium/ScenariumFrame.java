package demo.bandeirabr.scenarium;
...
import demo.component.ComponentsFrame;

public class ScenariumFrame extends HBContainer {
   ...
   * Construtor
   */
  public ScenariumFrame(int scenaId) {
    this.addElement(new HBPageHeader("Cenário " + scenaId,
      "Janeiro 2008 a Março 2008").addEventClick(new HMouseListener() {
        @Override
      ...
      HElement monthYearCol =
        new HBCol("md", 2).addElement(
          ...
          .addAttribute("title", "Selecione o Mês e o Ano")).addElement(
            new HBMobileSpace());
      this.addElement(new HBRow().addElement(viewCol).addElement(stateCol)
    ...
    for (int n = 0; n < 3; n++) {
      ...
      this.addElement(new HBPanel().setBackgroundColor(HBColor.INFO)
  ...
  protected void onExportAction(HMouseEvent e) {
    this.getBrowser().setRoot(new ComponentsFrame());
  }
  ...
  protected void onPrintAction(HMouseEvent e) {
    ...
    new Thread(new Runnable() {
      ...
      public void run() {
        ...
        getBrowser().sync(new IBrowserSync() {
          @Override
