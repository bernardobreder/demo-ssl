...
static void compute_key_schedule(const unsigned char *key, int key_lengte, unsigned char w[][4]) {
	int i;
...
void matrix_multiply(unsigned char matrix1[4][4], unsigned char matrix2[4][4], unsigned char target[4][4]) {
	int i;
	int j;
	for (i = 0; i < 4; i++) {
		for (j = 0; j < 4; j++) {
			target[i][j] = matrix1[i][0] * matrix2[0][j] + matrix1[i][1] * matrix2[1][j] + matrix1[i][2] * matrix2[2][j] + matrix1[i][3] * matrix2[3][j];
		}
	}
}
...
static void mix_columns(unsigned char s[][4]) {
	...
	for (i = 0; i < 4; i++) {
		temp[0] = inproduct(2, s[0][i]) ^ inproduct(3, s[1][i]) ^ s[2][i] ^ s[3][i];
		temp[1] = s[0][i] ^ inproduct(2, s[1][i]) ^ inproduct(3, s[2][i]) ^ s[3][i];
...
static void inversion_mix_columns(unsigned char s[][4]) {
	...
	unsigned char temp[4];
	for (i = 0; i < 4; i++) {
		temp[0] = inproduct(0x0e, s[0][i]) ^ inproduct(0x0b, s[1][i]) ^ inproduct(0x0d, s[2][i]) ^ inproduct(0x09, s[3][i]);
		temp[1] = inproduct(0x09, s[0][i]) ^ inproduct(0x0e, s[1][i]) ^ inproduct(0x0b, s[2][i]) ^ inproduct(0x0d, s[3][i]);
		temp[2] = inproduct(0x0d, s[0][i]) ^ inproduct(0x09, s[1][i]) ^ inproduct(0x0e, s[2][i]) ^ inproduct(0x0b, s[3][i]);
		temp[3] = inproduct(0x0b, s[0][i]) ^ inproduct(0x0d, s[1][i]) ^ inproduct(0x09, s[2][i]) ^ inproduct(0x0e, s[3][i]);
		s[0][i] = temp[0];
		s[1][i] = temp[1];
		s[2][i] = temp[2];
		s[3][i] = temp[3];
...
static void aes_block_decrypt(const unsigned char *input_block, unsigned char *output_block, const unsigned char *key, int key_size) {
	int i, j;
	int ronde;
	int number;
	unsigned char state[4][4];
...
static void aes_encrypt(const unsigned char *input, long input_len, unsigned char *output, const unsigned char *iv, const unsigned char *key, int key_length) {
...
static void aes_decrypt(const unsigned char *input, long input_len, unsigned char *output, const unsigned char *iv, const unsigned char *key, int key_length) {
	while (input_len >= AES_BLOCK_SIZE) {
...
void aes_128_encrypt(const unsigned char *plaintext, long plaintext_len, unsigned char ciphertext[], void *iv, const unsigned char *key) {
	aes_encrypt(plaintext, plaintext_len, ciphertext, iv, key, 16);
}

void aes_128_decrypt(const unsigned char *ciphertext, long ciphertext_len, unsigned char plaintext[], void *iv, const unsigned char *key) {
	aes_decrypt(ciphertext, ciphertext_len, plaintext, iv, key, 16);
}

void aes_256_encrypt(const unsigned char *plaintext, long plaintext_len, unsigned char ciphertext[], void *iv, const unsigned char *key) {
	aes_encrypt(plaintext, plaintext_len, ciphertext, iv, key, 32);
}

void aes_256_decrypt(const unsigned char *ciphertext, long ciphertext_len, unsigned char plaintext[], void *iv, const unsigned char *key) {
