 ...
 */
void add_magnitude( struct huge *h1, struct huge *h2 )
{
    unsigned long i, j;
    unsigned int sum;
...
static void subtract_magnitude( struct huge *h1, struct huge *h2 )
{
    long i = h1->size;
    long j = h2->size;
...
void set_huge( struct huge *h, unsigned int val )
{
    ...
    // up in chars (leave at least one byte, though, if ìvalî is 0).
    ...
    // byte (up to the first 0 byte) and copy it into the ìhugeî
...
void left_shift( struct huge *h1 )
{
    unsigned long i;
...
static void right_shift( struct huge *h1 )
{
    int i;
 ...
 */
void exponentiate( struct huge *h1, struct huge *exp )
{
    unsigned long i = exp->size;
    int mask;
    struct huge tmp1, tmp2;
...
{
    struct huge i, j, y2, y1, y, quotient, remainder, a_temp;
